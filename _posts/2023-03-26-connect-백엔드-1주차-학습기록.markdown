...
변수의 이름은 아래의 규칙을 따라야 한다.
1. 알파벳 대소문자, 0~9까지의 숫자, _, $ 이외의 문자는 사용할 수 없다.
2. 숫자로는 시작할 수 없다.
3. 키워드를 변수의 이름으로 사용할 수 없다.
...
3. 비교 연산: <, >, <=, >=, ==, != 연산자를 통해 값의 대소 비교 및 일치 여부를 확인할 수 있다. 연산의 결과는 true, false 중 하나이다. <=는 ≤, >=는 ≥와 같으며, ==는 비교할 두 값이 같으면 true, !=는 다르면 true이다. =가 아니라 ==임을 조심하자.
4. 논리 연산: &&, \|\|, ! 연산자를 통해 논리 연산을 수행할 수 있다. &&는 양쪽의 값이 모두 true이면 true, \|\|는 한 쪽이 true이면 true, !는 값이 true이면 false, false이면 true이다.
...
함수는 어떤 기능을 수행하기 위한 코드들을 모아놓은 것이다. 함수를 사용하면 코드의 가독성이 좋아지고 유지보수가 쉬워진다는 장점이 있다. 함수를 사용하기 위해서는 우선 함수를 선언해야 한다. 아래의 예시를 통해 함수의 선언을 살펴보면
...
function은 함수를 선언하기 위해 사용하는 키워드이다. add는 함수의 이름이고, 이를 통해 함수를 호출, 즉 사용할 수 있다. a와 b는 매개변수로, 함수의 동작에 필요한 값을 받는 변수이다. 이 함수에서는 두 수를 더하는 것이 목적이므로, 더할 두 개의 숫자를 a, b에 받는다. 이후 블럭 안에 이 함수가 실행할 코드를 작성하면 된다.

함수를 호출하면 해당 함수가 실행되며, 함수의 호출은 아래와 같은 형태로 이루어진다.
